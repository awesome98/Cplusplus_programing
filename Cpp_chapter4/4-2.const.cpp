...
int main(void){
    ...
    int i4 = 20;
	int *p1 = &i1;
    const int* pc_a = &i2; //pointer-to-constant => 포인터가 가르키는 값이 상수
    const int* pc_b = &i2;
    //(*pc_a)++;
    int* const cp_a = &i3; //constant pointer => 포인터가 상수
    int* const cp_b = &i3;
    (*cp_a)++;
    const int* const cpc_a = &i4; //constant pointer-to-const => 포인터가 가르키는 값이 상수, 포인터도 상수
    ...
    // pi = pc_a; <= pi에 할당된 주소값을 통해 pc_a가 가르키는 상수값을 변경할 수 있기 때문에 할당되지 않음.
    pc_a = pi;
